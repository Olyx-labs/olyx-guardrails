...
class NotifierTest < Minitest::Test
  ...
  def test_policy_redacts_every_untrusted_event_field
    ...
    result = violation_result(
      ...
      policy_violated: true,
      policy_findings: [{ rule: 'confidential_project', blocked: true }],
      llm_analysis: { reason: "#{restricted} #{credential} #{email}" }
    )
    ...
    assert event.key?(:llm_reason)
    refute event.key?(:ai_reason)
  end
  ...
  def test_rejects_invalid_result_and_metadata_arguments
    [nil, {}, { risk_score: 'high' }, { risk_score: Float::NAN }, { risk_score: 1.1 }].each do |result|
      assert_raises(ArgumentError) { notifier.notify(result) }
    end

    assert_raises(ArgumentError) do
      notifier.notify(violation_result, metadata: nil)
    end
  end
