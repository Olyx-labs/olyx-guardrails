...
class RailsPolicyFileTest < Minitest::Test
  ...
  def test_configuration_rejects_ambiguous_policy_sources
    configuration = Olyx::Guardrails::Rails::Configuration.new
    configuration.policy = Olyx::Guardrails::Policy.default
    configuration.policy_path = 'policy.yml'
  ...
  def test_configuration_is_immutable_after_finalization
    ...
    assert_raises(Olyx::Guardrails::ConfigurationError) { configuration.enabled = false }
  end
