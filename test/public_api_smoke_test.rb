...
class PublicApiSmokeTest < Minitest::Test
  ...
  # ---------------------------------------------------------------------
  # LLM provider hook contract
  # ---------------------------------------------------------------------

  # Stands in for any schema-model object that exposes `to_h`.
  FakeSchemaModel = Struct.new(:injection_attempt, :pii_detected, :secret_leaked, :risk_score, :reason)

  def test_llm_provider_receives_documented_context_keys
  ...
  def test_llm_provider_hash_return_is_merged_into_llm_analysis
    hook = ->(_text, _ctx) { { reason: 'looks fine', risk_score: 0.2 } }
    result = Olyx::Guardrails.check('hello world', llm_provider: hook)

    assert_equal 'looks fine', result[:llm_analysis][:reason]
  end

  def test_llm_provider_accepts_schema_model_object_via_to_h
    model = FakeSchemaModel.new(false, false, false, 0.1, 'schema-model reason')
    hook = ->(_text, _ctx) { model }
    result = Olyx::Guardrails.check('hello world', llm_provider: hook)

    assert_equal 'schema-model reason', result[:llm_analysis][:reason]
  end

  def test_llm_provider_can_add_violation_but_not_clear_one
    hook = ->(_text, _ctx) { { pii_detected: false } }
    result = Olyx::Guardrails.check('my email is user@example.com', llm_provider: hook)

    assert result[:pii_detected], 'LLM provider must not clear a deterministic finding'
  end

  def test_llm_provider_can_add_a_finding_regex_missed
  ...
  def test_llm_provider_non_boolean_finding_values_are_not_honored
    hook = ->(_text, _ctx) { { injection_attempt: 'true' } }
    result = Olyx::Guardrails.check('clean input', llm_provider: hook)
  ...
  def test_llm_provider_exception_is_captured_as_bounded_error
  ...
  def test_llm_provider_non_hash_non_model_return_is_captured_as_error
  ...
  def test_llm_provider_non_finite_risk_score_is_ignored_not_crashing
  ...
  def test_llm_failure_mode_block_adds_failed_llm_check_and_rejects
    policy = Olyx::Guardrails::Policy.new(llm_failure_mode: :block)
    hook = ->(_text, _ctx) { raise 'boom' }
    result = Olyx::Guardrails.check('clean input', policy: policy, llm_provider: hook)

    refute result[:allowed]
    llm_check = result[:checks].find { |check| check[:type] == 'llm' }

    assert_equal({ type: 'llm', allowed: false, error: true }, llm_check)
  end

  def test_llm_failure_mode_raise_raises_provider_error
    policy = Olyx::Guardrails::Policy.new(llm_failure_mode: :raise)
    hook = ->(_text, _ctx) { raise 'boom' }

    assert_raises(Olyx::Guardrails::LlmProviderError) do
      Olyx::Guardrails.check('clean input', policy: policy, llm_provider: hook)
    end
  end

  def test_llm_failure_mode_allow_is_the_default_and_swallows_errors
    hook = ->(_text, _ctx) { raise 'boom' }
    result = Olyx::Guardrails.check('clean input', llm_provider: hook)

    assert result[:allowed]
    refute(result[:checks].any? { |check| check[:type] == 'llm' })
  end
