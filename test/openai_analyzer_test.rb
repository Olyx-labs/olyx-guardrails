...
class OpenAIAnalyzerTest < Minitest::Test
  ...
  def test_connector_uses_responses_schema_model_and_merges_parsed_result
    ...
    refute responses.params[:store]
    assert_equal({ timeout: 5 }, responses.params[:request_options])
    assert_in_delta(0.0, responses.params[:temperature])
  ...
  def test_default_schema_and_client_use_the_official_sdk_contract
    ...
    components = Olyx::Guardrails::Integrations::OpenAIComponents
    schema_registry = components::SchemaRegistry
    sdk = components::Sdk
    original_schema = schema_registry.const_get(:AnalysisSchema, false) if
      schema_registry.const_defined?(:AnalysisSchema, false)
    schema_registry.send(:remove_const, :AnalysisSchema) if original_schema
    ...
    schema_registry.send(:remove_const, :AnalysisSchema) if schema_registry&.const_defined?(:AnalysisSchema, false)
    schema_registry.const_set(:AnalysisSchema, original_schema) if original_schema
  ...
  def test_missing_optional_sdk_has_an_actionable_error
    analyzer_class = Olyx::Guardrails::Integrations::OpenAIAnalyzer
    sdk = Olyx::Guardrails::Integrations::OpenAIComponents::Sdk

    sdk.stub(:require, ->(_name) { raise LoadError }) do
