...
class OperationalApiTest < Minitest::Test
  ...
  def test_llm_failure_modes_are_explicit
    allowed = Olyx::Guardrails.check('safe', llm_provider: failing_analyzer)
    blocked_policy = Olyx::Guardrails::Policy.new(llm_failure_mode: :block)
    blocked = Olyx::Guardrails.check('safe', policy: blocked_policy, llm_provider: failing_analyzer)
    raising_policy = Olyx::Guardrails::Policy.new(llm_failure_mode: :raise)

    assert allowed[:allowed]
    refute blocked[:allowed]
    assert_equal 'llm', blocked[:checks].last[:type]
    assert_raises(Olyx::Guardrails::LlmProviderError) do
      Olyx::Guardrails.check('safe', policy: raising_policy, llm_provider: failing_analyzer)
    end
  end
  ...
  def test_invalid_llm_failure_mode_is_rejected
    assert_raises(ArgumentError) { Olyx::Guardrails::Policy.new(llm_failure_mode: :retry_forever) }
  end

  def test_legacy_ai_failure_mode_is_not_accepted
    assert_raises(ArgumentError) { Olyx::Guardrails::Policy.new(ai_failure_mode: :block) }
  end
end
