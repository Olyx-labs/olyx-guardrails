...
class OlyxGuardrailsInheritedJob < OlyxGuardrailsProtectedJob; end
...
class RailsIntegrationTest < ActionDispatch::IntegrationTest
  ...
  def test_reusable_enforcer_covers_service_entry_points
    ...
    error = assert_raises(Olyx::Guardrails::Blocked) do
      Olyx::Guardrails::Rails::Enforcer.check!('Project Falcon')
    end

    assert_equal 'content blocked by guardrail policy', error.message
  end
  ...
  def test_active_job_concern_checks_declared_arguments
    ...
    assert_equal 'Explain Ruby', OlyxGuardrailsInheritedJob.perform_now('Explain Ruby')
    assert_raises(Olyx::Guardrails::Blocked) do
  ...
  def test_active_job_selectors_fail_during_declaration
    job_class = Class.new(ActiveJob::Base) do
      include Olyx::Guardrails::Rails::Job
    end

    [[], [-1], ['prompt']].each do |selectors|
      assert_raises(ArgumentError) { job_class.guardrails_input_arguments(*selectors) }
    end
  end

  def test_rails_entry_points_reject_non_hash_metadata_consistently
    calls = [
      -> { Olyx::Guardrails::Rails.check('safe', metadata: nil) },
      -> { Olyx::Guardrails::Rails.check_messages([], metadata: nil) },
      -> { Olyx::Guardrails::Rails.check_output('safe', metadata: nil) },
      -> { Olyx::Guardrails::Rails::Upload.check('safe', extractor: ->(value) { value }, metadata: nil) },
      -> { OlyxGuardrailsGraphqlResolver.new.send(:guardrails_check_graphql!, 'safe', metadata: nil) },
      -> { OlyxGuardrailsChannel.new.send(:guardrails_check_cable!, 'safe', metadata: nil) }
    ]

    calls.each do |call|
      error = assert_raises(ArgumentError, &call)
      assert_equal 'guardrail metadata must be a Hash', error.message
    end
  end
